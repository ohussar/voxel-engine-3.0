...
public class MemoryLoader {
    ...
    public int updateVAO(int VAO, long[] compressedData){
        if(VAO == -1){
            VAO = createVAO();
        }else{
            GL30.glDeleteVertexArrays(VAO);
    ...
    private void storeDataInAttributeList(long[] data, int attributeNumber, int dimentions) {
        int vboID = GL15.glGenBuffers();
        //vbos.add(vboID);
        GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vboID);
        LongBuffer buffer = this.toLongBuffer(data);
        GL15.glBufferData(GL15.GL_ARRAY_BUFFER, buffer, GL15.GL_STATIC_DRAW); // binding the buffer
        // ONLY DOUBLE HAS 64 BIT
        GL20.glVertexAttribPointer(attributeNumber, dimentions, GL11.GL_DOUBLE, false, 0, 0);
        GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, 0); // unbinding the buffer
    }
    ...
    private LongBuffer toLongBuffer(long[] data){
